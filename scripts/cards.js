...
function createCards() {
  ...
  const sortCards = [...arrayCardsName, ...arrayCardsName].sort(
    () => Math.random() - 0.5
  );
...
function checkGameWin() {
  ...
  if (disabledCards.length === 24) {
    ...
    if (storageRank) {
      const rankDate = [...storageRank, userDate];
      localStorage.setItem("@memoryGame:rank", JSON.stringify(rankDate));
    } else {
      localStorage.setItem("@memoryGame:rank", JSON.stringify([userDate]));
    }
